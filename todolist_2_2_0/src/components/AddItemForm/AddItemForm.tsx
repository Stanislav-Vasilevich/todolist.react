...
const AddItemForm: React.FC<PropsType> = ({todolistId, createItem, minCharter, maxCharter}) => {
  ...
  return (
    <div className={s.block}>
      ...
      {
        (title.length <= maxCharter && title.length >= minCharter)
        && <div className={s.lengthText}>max length ${maxCharter} charters. <span
          className={s.length}>Now: {title.length}</span></div>
      }
      {
        title.length > maxCharter
        && <div className={s.errorText}>max length ${maxCharter} charters <span
        className={s.lengthError}>Now: {title.length}</span></div>
      }
    </div>
  );
};
